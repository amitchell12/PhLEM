...
%Skip data before '"3' for ECG data.
index=1;IsHeader=1;
while IsHeader==1
  header(index)=textscan(fid,'%s',1);  %textscan() is faster than textread()
  if strcmp(header{index}(1),'"3') 
    IsHeader=0;
  end
  index=index+1;
end

%% THIS CUTS OFF EARLY - WHY?
data = [];
j = 0;
while 1
    t = fgetl(fid);	 %Read data until end of u16 data.
    if(~ischar(t)),break,end
    j = j+1;
    data{j} = t;
end
%data = textscan(fid,'%s');   %Read in remaining data (time stamps and statistics).
[footer pos] = textscan(fid,'%s');
%footer2 = textscan(extractAfter(fid, pos), '%s');   
